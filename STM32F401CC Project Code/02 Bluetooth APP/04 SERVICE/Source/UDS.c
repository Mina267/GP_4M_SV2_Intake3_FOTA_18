...
static void SID_RequestDownload(void)
{
	...
	if (SecurityLVLOneStatus == SECURITY_UNLOCKED)
	{
		...
		if (RequestDownload_Flag == FALSE)
		{
			...
			if (((START_OF_BANK_1 == StartAddress) && ((UDS_BANK1_ACTIVE == UDS_Data.ActiveBank) || (UDS_NO_VALID_BANK == UDS_Data.ActiveBank)))
				|| ((START_OF_BANK_2 == StartAddress) && (UDS_BANK2_ACTIVE == UDS_Data.ActiveBank)))
			{
